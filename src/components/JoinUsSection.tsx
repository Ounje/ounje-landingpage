...
const JoinUsSection = () => {
  ...
  return (
    <section id="joinUs" className="py-20 md:py-28 px-4 md:px-8 bg-[#ECFFED] overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {roles.map((role, i) => (
            ...
            >
              ...
              <div className="relative flex justify-center pt-8 px-6 overflow-hidden">
                {/* Soft glow behind image */}
                <div
                  className="absolute inset-x-0 bottom-0 h-24 pointer-events-none"
                  style={{ background: `linear-gradient(to top, ${role.bg}, transparent)` }}
                />
                <motion.img
                  src={role.image}
              ...
              {/* Card body */}
              <div className="flex flex-col flex-1 px-7 pb-8 pt-4">
                {/* Icon + title */}
                <div className="flex items-center gap-3 mb-3">
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${role.iconBg}`}>
                    <role.Icon className={`w-5 h-5 ${role.iconColor}`} />
                  </div>
                  <div>
                    <p className={`text-[10px] font-bold uppercase tracking-[0.18em] ${role.descColor}`}>
                      Join as
                    </p>
                    <h3 className={`text-xl font-extrabold leading-tight ${role.textColor}`}>
                      {role.title}
                    </h3>
                  </div>
                </div>
                ...
                {/* Tagline */}
                <p className={`text-lg font-bold mb-2 leading-snug ${role.textColor}`}>
                  {role.tagline}
                </p>

                {/* Desc */}
                <p className={`text-sm leading-relaxed mb-6 flex-1 ${role.descColor}`}>
                  {role.desc}
                </p>
