...
import { Store, ShieldCheck, Mail, Lock, Phone, ArrowLeft, Eye, EyeOff } from "lucide-react";
...
export default function VendorAuthPage() {
  ...
  const [speciality, setSpeciality] = useState("Swallows");

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In future this will update useAuthStore, for now we mock redirect
    alert(`Successfully ${isLogin ? "logged in" : "registered"} as a Vendor! Routing to dashboard...`);
          ...
          <div className="md:col-span-7 bg-[#ECFFED]/20 rounded-2xl p-6 md:p-8 border border-[#2C5E2E]/5">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {!isLogin && (
                <>
                  <div>
                    <label className="block text-xs font-bold text-[#1A3F1C] uppercase tracking-wider mb-2">Buka/Kitchen Name</label>
                    <div className="relative flex items-center">
                      <Store className="absolute left-4 w-4 h-4 text-gray-400" />
                      <input
                        type="text"
                        required
                        placeholder="e.g. Mama Buka"
                        value={bukaName}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-[#1A3F1C] uppercase tracking-wider mb-2">Kitchen Speciality</label>
                    <select
                      value={speciality}
                    ...
                    >
                      <option value="Swallows">Swallows & Soups (Amala, Pounded Yam)</option>
                      <option value="Rice">Rice Dishes (Jollof, Ofada, Fried Rice)</option>
                      <option value="Grills">Grills & Suya (Chicken, Beef, Fish)</option>
                      <option value="Stews">Pepper Stews & Native Dishes</option>
                    </select>
                  </div>
              ...
              >
                {isLogin ? "Log In as Vendor" : "Register Kitchen & Start Selling"}
              </button>
            </form>
          </div>
